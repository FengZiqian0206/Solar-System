import { THREE, context } from './renderer-support.js?v=follow-scale-4';
...
function updateBodyDetails(){
  ...
  const beltInner=VISUAL_HALF*.300,beltOuter=VISUAL_HALF*.352,beltHeight=Math.max(1,VISUAL_HALF*.10);
...
const vertexShader=`
attribute float aSeed,aDetail; uniform float uTime,uHalf,uPixelRatio,uDenseMode,uSpaceScale,uZoom; uniform vec2 uViewport; uniform vec4 uBodies[9]; uniform vec2 uLevels[9]; uniform vec3 uSaturn; uniform vec3 uShadowDirs[9]; varying float vRatio,vSeed,vPerspective,vVisible,vBodyDepth,vShadow;
...
function followFitZoom(){const b=bodies[followPlanet+1];return b?Math.min(28,78/Math.max(1.25,b.radius*7.5)):1}
