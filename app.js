...
const vertexShader=`
attribute float aSeed,aDetail; uniform float uTime,uHalf,uPixelRatio,uDenseMode,uSpaceScale,uZoom,uBeltBoost; uniform vec2 uViewport; uniform vec4 uBodies[9]; uniform vec2 uLevels[9]; uniform vec3 uSaturn; uniform vec3 uShadowDirs[9]; varying float vRatio,vSeed,vPerspective,vVisible,vBodyDepth,vShadow,vEmphasis;
...
void main(){
 ...
 float beltShift=visualDense*(beltHash-.5)*uHalf*.012;float beltInner=uHalf*mix(.455,.265,compactLayer);float beltOuter=uHalf*mix(.515,.320,compactLayer);float beltHeight=max(1.,uHalf*mix(.10,.035+beltClump*.020,visualDense));float beltChance=mix(.48,.30+beltClump*.12,visualDense)+uBeltBoost*(.32+beltClump*.12);
