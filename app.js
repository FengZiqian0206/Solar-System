import { THREE, context } from './renderer-support.js?v=solar-shadows-3';
...
void main(){
 ...
 float bodyDepth=0.,shadow=0.;for(int i=0;i<9;i++){vec3 d=position-uBodies[i].xyz;float dist=length(d);if(dist<uBodies[i].w){float inward=1.-dist/uBodies[i].w;bodyDepth=max(bodyDepth,inward);ratio=max(ratio,uLevels[i].y+(uLevels[i].x-uLevels[i].y)*pow(inward,.72));}if(i>0){vec3 shadowDir=uShadowDirs[i];float behind=dot(d,shadowDir);if(behind>0.){vec3 radial=d-shadowDir*behind;float shadowRadius=uBodies[i].w*1.75+.95+behind*.03;float softness=1.-smoothstep(shadowRadius*shadowRadius*.16,shadowRadius*shadowRadius,dot(radial,radial));float distanceFade=1.-smoothstep(uBodies[i].w*.35,uHalf*1.35,behind);shadow=max(shadow,softness*distanceFade);}}}
