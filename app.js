import { THREE, context } from './renderer-support.js?v=solar-shadows-2';
...
const vertexShader=`
attribute float aSeed,aDetail; uniform float uTime,uHalf,uPixelRatio,uDenseMode,uSpaceScale; uniform vec2 uViewport; uniform vec4 uBodies[9]; uniform vec2 uLevels[9]; uniform vec3 uSaturn; uniform vec3 uShadowDirs[9]; varying float vRatio,vSeed,vPerspective,vVisible,vBodyDepth,vShadow;
...
void main(){
 ...
 float bodyDepth=0.,shadow=0.;for(int i=0;i<9;i++){vec3 d=position-uBodies[i].xyz;float dist=length(d);if(dist<uBodies[i].w){float inward=1.-dist/uBodies[i].w;bodyDepth=max(bodyDepth,inward);ratio=max(ratio,uLevels[i].y+(uLevels[i].x-uLevels[i].y)*pow(inward,.72));}if(i>0){vec3 shadowDir=uShadowDirs[i];float behind=dot(d,shadowDir);if(behind>0.){vec3 radial=d-shadowDir*behind;float shadowRadius=uBodies[i].w*(1.55+behind/uHalf*.75)+.95;float softness=1.-smoothstep(shadowRadius*shadowRadius*.18,shadowRadius*shadowRadius,dot(radial,radial));float distanceFade=1.-smoothstep(uHalf*.60,uHalf*2.2,behind);shadow=max(shadow,softness*mix(.65,1.,distanceFade));}}}
